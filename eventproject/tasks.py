...
def kazexpo_import_job():
    """Celery-обёртка над ЕДИНСТВЕННОЙ реализацией импорта в eventproject.cron.

    Ранее здесь дублировалась логика с другим event pk (777 против 797 в cron.py),
    из-за чего было два расходящихся источника правды. Теперь источник один —
    eventproject.cron.kazexpo_import_job. Если включите Celery beat, задача
    выполнит ровно ту же логику, что и django-crontab (без дубля и без расхождений).
    """
    from eventproject.cron import kazexpo_import_job as _run_import
    return _run_import()