    ...
    try:
        ...
        if req.status == "Sent":
            ...
            context_dict["delete_message"] = (
                "Attention! Sending the application does not guarantee entrance to the event"
            )
...
def add_attendee(request, request_id):
    ...
    if request.method == "GET":
        ...
        if doc_start > date.today():
            ...
            context_dict["delete_message"] = (
                "Size of the photo is required to be at least 50Kb"
            )
        elif attendee.docScan.size < 50000:
            context_dict["delete_message"] = (
                "Size of the document image is required to be at least 50Kb"
            )
        elif check_dublicate(attendee, req):
            context_dict["delete_message"] = (
                "Duplicate entry. You have already added this person"
            )
        else:
            attendee.save()
            context_dict["success_message"] = (
                "Attendee "
                + attendee.transcription
                + " is successfully added to the request"
            )
        context_dict["req"] = req
